...
output_dir = os.path.join(current_dir, "scenario_zero_test_999")
...
class Object:
    ...
    def get_log_str(self, force_zero=False):
        """10% 확률로 호출되어 0 데이터를 생성함"""
        if force_zero:
            return "0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0"
...
for i in range(1, NUM_SCENARIOS + 1):
    ...
    while running:
        ...
        obj_log = main_obj.get_log_str(force_zero=is_missed)
        frame_header = f"FRAME: {frame_count}, MOTION: {motion_str}, OBJS:"
        scenario_logs.append(f"{frame_header} | {obj_log}")
        
        main_color = (255, 255, 255) if will_enter else (255, 200, 0)
        
        if is_missed:
            pygame.draw.rect(screen, (255, 0, 0), (main_obj.x, main_obj.y, main_obj.w, main_obj.h), 4)
        else:

...
for i in range(1, NUM_SCENARIOS + 1):
    ...
    while True:
        # --- [추가] 매 프레임 속도 벡터 변경 (Dynamic Velocity) ---
        accel_range = 0.2
        vx += np.random.uniform(-accel_range, accel_range)
        vy += np.random.uniform(-accel_range, accel_range)
        
        # 속도 제한 (너무 빠르거나 멈추는 것 방지)
        speed = np.sqrt(vx**2 + vy**2)
        if speed > 5.0:
            vx = (vx / speed) * 5.0
            vy = (vy / speed) * 5.0
        elif speed < 1.0:
            # 최소 속도 유지 (방향은 유지하되 속력만 보정)
            vx = (vx / (speed + 1e-6)) * 1.5
            vy = (vy / (speed + 1e-6)) * 1.5
        ...
        if curr_id != original_id:
            ...
            if np.random.rand() < 0.01:
                curr_id = np.random.randint(1, 201)
            
        # 데이터 기록
        log_str = f"{curr_id}, {curr_x:.6f}, {curr_y:.6f}, {curr_w:.6f}, {curr_h:.6f}"
        log_data.append(log_str)

        # 5. 종료 조건: 타겟 영역(180~300) 안으로 진입
